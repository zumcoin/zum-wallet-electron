...
const ZUMELECTRON_VERSION = app.getVersion() || '1.1.x';
...
const DEFAULT_SETTINGS = {
    ...
    pubnodes_custom: ['127.0.0.1:17935'],
...
const WIN_TITLE = `${config.appName} ${ZUMELECTRON_VERSION} - ${config.appDescription}`;
...
log.info(`Starting ZumElectron ${ZUMELECTRON_VERSION}`);
...
function createWindow() {
    ...
    let bgColor = darkmode ? '#000000' : '#02853E1A';

    const winOpts = {
        title: WIN_TITLE,
        icon: path.join(__dirname, 'src/assets/zumelectron_icon.png'),
        frame: true,
...
function initSettings() {
    ...
    settings.set('version', ZUMELECTRON_VERSION);
    serviceBinCheck();
