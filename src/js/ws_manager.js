...
const ZumElectronSession = require('./ws_session');
const ZumElectronApi = require('./ws_api');
...
const wsession = new ZumElectronSession(sessConfig);
...
var ZumElectronManager = function () {
    if (!(this instanceof ZumElectronManager)) {
        return new ZumElectronManager();
    }
...
ZumElectronManager.prototype.init = function () {
    this._getSettings();
    ...
    this.serviceApi = new ZumElectronApi(cfg);
};

ZumElectronManager.prototype._getSettings = function () {
    let nodeAddress = settings.get('node_address').split(':');
    this.daemonHost = nodeAddress[0] || null;
    this.daemonPort = nodeAddress[1] || null;
    this.serviceBin = settings.get('service_bin');
...
ZumElectronManager.prototype._reinitSession = function () {
    this._wipeConfig();
    wsession.reset();
    this.notifyUpdate({
        type: 'sectionChanged',
...
ZumElectronManager.prototype._serviceBinExists = function () {
    wsutil.isFileExist(this.serviceBin);
};

// check 
ZumElectronManager.prototype.serviceStatus = function () {
    return (undefined !== this.serviceProcess && null !== this.serviceProcess);
};

ZumElectronManager.prototype.isRunning = function () {
    this.init();
...
ZumElectronManager.prototype._writeIniConfig = function (cfg) {
...
ZumElectronManager.prototype._writeConfig = function (cfg) {
    let configFile = wsession.get('walletConfig');
    if (!configFile) return '';
...
ZumElectronManager.prototype._wipeConfig = function () {
    try { fs.unlinkSync(wsession.get('walletConfig')); } catch (e) { }
};

ZumElectronManager.prototype.startService = function (walletFile, password, onError, onSuccess, onDelay) {
    this.init();
...
ZumElectronManager.prototype._argsToIni = function (args) {
    let configData = "";
...
ZumElectronManager.prototype._spawnService = function (walletFile, password, onError, onSuccess, onDelay) {
    this.init();
...
ZumElectronManager.prototype.stopService = function () {
...
ZumElectronManager.prototype.terminateService = function (force) {
...
ZumElectronManager.prototype.startSyncWorker = function () {
    this.init();
    let wsm = this;
...
ZumElectronManager.prototype.stopSyncWorker = function () {
...
ZumElectronManager.prototype.getNodeFee = function () {
    let wsm = this;
...
ZumElectronManager.prototype.genIntegratedAddress = function (paymentId, address) {
    let wsm = this;
...
ZumElectronManager.prototype.createWallet = function (walletFile, password) {
    this.init();
    let wsm = this;
    return new Promise((resolve, reject) => {
...
ZumElectronManager.prototype.importFromKeys = function (walletFile, password, viewKey, spendKey, scanHeight) {
    this.init();
    let wsm = this;
    return new Promise((resolve, reject) => {
...
ZumElectronManager.prototype.importFromSeed = function (walletFile, password, mnemonicSeed, scanHeight) {
    this.init();
    let wsm = this;
    return new Promise((resolve, reject) => {
        scanHeight = scanHeight || 0;
...
ZumElectronManager.prototype.getSecretKeys = function (address) {
    let wsm = this;
    return new Promise((resolve, reject) => {
...
ZumElectronManager.prototype.sendTransaction = function (params) {
    let wsm = this;
    return new Promise((resolve, reject) => {
...
ZumElectronManager.prototype.rescanWallet = function (scanHeight) {
    let wsm = this;
...
ZumElectronManager.prototype._fusionGetMinThreshold = function (threshold, minThreshold, maxFusionReadyCount, counter) {
    let wsm = this;
    return new Promise((resolve, reject) => {
        counter = counter || 0;
...
ZumElectronManager.prototype._fusionSendTx = function (threshold, counter) {
    let wsm = this;
...
ZumElectronManager.prototype.optimizeWallet = function () {
    let wsm = this;
...
ZumElectronManager.prototype.networkStateUpdate = function (state) {
    ...
    if (state === 0) {
        ...
        this.init();
        // looks like zum-service always stalled after disconnected, just kill & relaunch it
...
ZumElectronManager.prototype.notifyUpdate = function (msg) {
    uiupdater.updateUiState(msg);
};

ZumElectronManager.prototype.resetState = function () {
    return this._reinitSession();
};

module.exports = ZumElectronManager;
